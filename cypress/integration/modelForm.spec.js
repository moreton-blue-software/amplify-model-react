...
describe('check seed data', async () => {
  ...
  it('save form', function() {
    ...
    cy.getClient()
      ...
      .then(function() {
        cy.contains('#log-tracer', `"savedId": "${savedVacancy.id}"`);
      });
  });
  it('update form (after save)', function() {
  ...
  it('update form (after reload)', function() {
    expect(savedVacancy.id).to.exist;
    cy.visit('/model/load-model-from-id?id=' + savedVacancy.id);

    const changedPosition = 'hello22';
    cy.contains(`id:${savedVacancy.id}`);
    cy.contains('#log-tracer', '"videoRenderUrl": "http');
    cy.get('input[id="position"]').then(function(inp) {
      expect(inp.val()).not.eq(changedPosition);
    });
    cy.get('input[id="position"]').clear();
    cy.get('input[id="position"]').type(changedPosition, { delay: 100 });
    const fileName = 'newn' + nanoid();
